#!/usr/bin/env python3
"""Run matched C0 vs M1 sparse-recovery MultiAgentBench experiments."""
...
"""
PLANNER_PROMPT = "Act as planner. Read TASK.md and AGENTS.md. Write plan.md covering architecture, every requirement, dependencies, deterministic tests, and edge cases. Do not implement code. Keep chat under five lines."
...
def run_one(root: Path, item: dict[str, Any], condition: str, repetition: int) -> dict[str, Any]:
    ...
    if impl_blocker:
        try:
            impl_recovery_text = recovery_prompt(impl_blocker, condition)
        except Exception as exc:
    ...
    if review_blocker:
        try:
            review_recovery_text = recovery_prompt(review_blocker, condition)
        except Exception as exc:
...
def main() -> int:
    ...
    parser.add_argument("--condition", choices=("C0", "M1", "M2", "M3", "both", "c0m3"), default="both")
    parser.add_argument("--root", type=Path, default=DEFAULT_ROOT)
    args = parser.parse_args()
    root = args.root.expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    dataset = load_tasks()
    conditions = (["C0", "M1"] if args.condition == "both" else
                  ["C0", "M3"] if args.condition == "c0m3" else [args.condition])
    results = []
    failures = []
    for repetition in range(1, args.repetitions + 1):
        for task_id in parse_task_ids(args.tasks):
            ordered_conditions = conditions
            if args.condition in {"both", "c0m3"} and (task_id + repetition) % 2:
                ordered_conditions = list(reversed(conditions))
